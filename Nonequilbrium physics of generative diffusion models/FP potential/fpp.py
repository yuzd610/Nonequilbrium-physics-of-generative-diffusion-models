...
def sample_joint(mu, sigma2, size):
    ...
    X0_prime_samples = norm.rvs(loc=X0_prime_components * mu, scale=np.sqrt(sigma2), size=size)
    Xt_samples = norm.rvs(loc=X0_prime_samples * np.exp(-t), scale=np.sqrt(1 - np.exp(-2 * t)), size=size)
